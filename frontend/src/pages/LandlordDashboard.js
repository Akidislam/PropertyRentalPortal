...
const LandlordDashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const activeRents = rentalsRes.data.filter(r => r.status === 'Approved');
      const earnings = activeRents.reduce((acc, curr) => acc + (Number(curr.propertyPrice) || 0), 0);

      setStats({
        properties: myProperties.length.toString(),
        activeRents: activeRents.length.toString(),
        netEarnings: `৳${earnings.toLocaleString()}`
      });
    } catch (error) {
  ...
  const fetchUpdatedUser = async () => {
    try {
      ...
      showToast('error', '⚠️ Authentication desynchronized. Please re-verify.');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (!localUser || localUser.role !== 'landlord') {
      showToast('error', '🛑 Access restricted. Landlord credentials required.');
      navigate('/login');
    } else {
      fetchUpdatedUser();
    }
  ...
  const statCards = [
    { title: 'Properties', value: stats.properties, icon: <FaHome />, color: 'bg-blue-50 text-blue-600' },
    { title: 'Active Rents', value: stats.activeRents, icon: <FaUsers />, color: 'bg-green-50 text-green-600' },
    { title: 'Wallet Balance', value: user?.walletcoin || '0', icon: <FaWallet />, color: 'bg-amber-50 text-amber-600' },
    { title: 'Net Earnings', value: stats.netEarnings, icon: <FaChartLine />, color: 'bg-purple-50 text-purple-600' },
  ];

  const actionCards = [
    { name: 'Add Listing', path: '/add-property', icon: <FaPlus />, desc: 'Register a new premium asset' },
    { name: 'Approvals', path: '/landlord-approval-rental', icon: <FaUsers />, desc: 'Review incoming tenant requests' },
    { name: 'Wallet Core', path: '/landlord-wallet', icon: <FaWallet />, desc: 'Manage your digital currency' },
    { name: 'Rent History', path: '/payment-history', icon: <FaHistory />, desc: 'Audit your incoming revenue' },
    { name: 'Coin Logs', path: '/wallet-history', icon: <FaHistory />, desc: 'Track wallet transaction data' },
    { name: 'My Profile', path: '/my-profile', icon: <FaUser />, desc: 'View your full property management record' },
    { name: 'Feedback', path: '/submit-review', icon: <FaStar />, desc: 'Rate your rental experience' },
  ];
