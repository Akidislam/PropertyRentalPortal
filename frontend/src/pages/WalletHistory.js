...
import { FaMoneyBillWave, FaCreditCard, FaHistory, FaFilter, FaArrowUp, FaArrowDown, FaExchangeAlt } from 'react-icons/fa';
                ...
                {filteredTransactions.map((t) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-8">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl shadow-sm ${t.transactionType === 'topup' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'
                        }`}>
                        {getPaymentMethodIcon(t.paymentMethod)}
                      </div>
                      <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-[0.2em] ${t.status === 'completed' ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'
                        }`}>
                        {t.status}
                      </span>
                    </div>
